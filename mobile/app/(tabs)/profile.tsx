  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        ...
        if (token) {
           // 👇 FIX YAHAN HAI: URL ko /courses/my-courses kar diya
          ...
          if (response.ok) {
            ...
            // Backend return karta hai array of enrolled courses
            const count = data.length || 0;
            setEnrolledCount(count < 10 && count > 0 ? `0${count}` : count === 0 ? '00' : count);
          } else {
             // Agar error aaye toh log mein dekhne ke liye
             const errorData = await response.json();
             console.log("Failed to fetch count:", errorData);
          }
        }
      } catch (error) {
