...
export default function AddLessonPage() {
  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
      try {
        ...
        setTimeout(() => setLoading(false), 600); // Cinematic delay
      }
    };
    fetchCourses();
  ...
  const handleMouseMoveCard = (e: React.MouseEvent<HTMLFormElement>) => {
    ...
    <div className="min-h-screen pt-24 pb-12 bg-[#010206] text-slate-50 flex items-center justify-center font-sans selection:bg-emerald-500/30 selection:text-emerald-200 overflow-hidden relative px-4 sm:px-6 lg:px-8 perspective-[2000px]">
      ...
      <div className="max-w-3xl w-full mx-auto relative z-10 py-12">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-10 text-center">
          ...
          <p className="text-slate-400 font-light text-[15px] sm:text-[17px] mix-blend-screen">Enrich your course with high-quality content and structured video lectures.</p>
        </motion.div>
        ...
        >
          ...
          <div className="relative z-10 w-full h-full transform-gpu" style={{ transform: "translateZ(20px)" }}>
            ...
            {/* Header Mini */}
            <div className="flex items-center gap-4 sm:gap-5 mb-8 sm:mb-10 pb-6 sm:pb-8 border-b border-white/[0.04]">
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-[1rem] bg-gradient-to-br from-[#060d20] to-[#040814] border border-white/[0.08] flex items-center justify-center text-emerald-400 shadow-[0_8px_16px_rgba(0,0,0,0.4),inset_0_1px_1px_rgba(255,255,255,0.05)] shrink-0">
                <svg className="w-6 h-6 sm:w-7 sm:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                </svg>
              </div>
              <div>
                <h3 className="text-xl sm:text-2xl font-black text-white tracking-tighter">Module Details</h3>
                <p className="text-slate-500 text-[10px] sm:text-[11px] mt-1 font-black uppercase tracking-[0.2em]">Structure Configuration</p>
              </div>
            </div>
            ...
            <div className="space-y-6 sm:space-y-8">
              ...
              {/* Course Selection */}
              <div>
                <label className="block text-[11px] sm:text-[12px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Target Course</label>
                <div className="relative group">
                  ...
                  <div className="absolute inset-y-0 right-0 pr-4 sm:pr-5 flex items-center pointer-events-none z-20">
                    <svg className="h-4 w-4 sm:h-5 sm:w-5 text-slate-400 group-focus-within:text-emerald-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 sm:gap-8">
                ...
                <div className="md:col-span-1">
                  <label className="block text-[11px] sm:text-[12px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Index</label>
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
                      <svg className="h-5 w-5 text-slate-500 group-focus-within:text-emerald-400 transition-colors duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="md:col-span-3">
                  <label className="block text-[11px] sm:text-[12px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Module Title</label>
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
                      <svg className="h-5 w-5 text-slate-500 group-focus-within:text-emerald-400 transition-colors duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {/* Video URL */}
              <div>
                <label className="block text-[11px] sm:text-[12px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Video URL <span className="text-slate-500 lowercase tracking-normal font-medium">(Optional)</span></label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none z-10">
                    <svg className="h-5 w-5 text-slate-500 group-focus-within:text-emerald-400 transition-colors duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {/* Content / Notes */}
              <div>
                <label className="block text-[11px] sm:text-[12px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Study Material</label>
                <div className="relative group">
              ...
              <div className="pt-4 sm:pt-6">
                <button 
                  type="submit" 
                  disabled={submitting}
                  className={`group relative w-full sm:w-auto px-10 sm:px-12 py-4 sm:py-5 rounded-[1.25rem] sm:rounded-[1.5rem] text-[13px] sm:text-[15px] font-black uppercase tracking-widest transition-all duration-500 overflow-hidden flex justify-center items-center gap-3 border border-white/10 ${
                    submitting 
                      ? 'bg-emerald-900/50 cursor-not-allowed text-slate-400' 
                      : 'text-[#010206] bg-gradient-to-b from-emerald-400 to-teal-500 sm:hover:scale-[1.03] shadow-[0_0_30px_-5px_rgba(52,211,153,0.6),inset_0_1px_1px_rgba(255,255,255,0.8)] active:scale-95'
                  }`}
                >
                  {!submitting && <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out hidden sm:block"></div>}
                  <span className="relative z-10 flex items-center gap-2">
                    {submitting ? (
                      ...
                      <>
                        Publish Lesson
                        <svg className="w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" /></svg>
                      </>
                    )}
                  </span>
                </button>
