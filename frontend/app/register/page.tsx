...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Extract exactly what is needed and add enrolledCourses
      const userData = {
        _id: data._id,
        name: data.name,
        email: data.email,
  ...
  return (
    <main className="min-h-screen bg-[#010206] text-slate-50 flex items-center justify-center font-sans selection:bg-emerald-500/30 selection:text-emerald-200 overflow-hidden relative px-4 sm:px-6 lg:px-8 py-24 sm:py-32 perspective-[2000px]">
      ...
      >
        ...
        <div className="relative z-10 w-full h-full transform-gpu" style={{ transform: "translateZ(20px)" }}>
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-[13px] sm:text-[15px] text-slate-400 mt-2 font-light leading-relaxed mix-blend-screen">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className={`group relative w-full flex justify-center items-center py-4 px-4 mt-6 sm:mt-8 rounded-[1rem] sm:rounded-[1.25rem] text-[13px] sm:text-[15px] font-black uppercase tracking-[0.15em] sm:tracking-widest transition-all duration-500 overflow-hidden border border-white/10 ${
                loading 
                  ? 'bg-emerald-900/50 cursor-not-allowed text-slate-400' 
                  : 'text-[#010206] bg-gradient-to-b from-emerald-400 to-teal-500 hover:scale-[1.03] shadow-[0_0_30px_-10px_rgba(52,211,153,0.5)] active:scale-95'
              }`}
            >
